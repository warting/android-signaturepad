...
internal class SvgPoint {
    ...
    constructor(point: TimedPoint) {
        ...
        require(!point.x.isNaN()) {
            "Cannot create SvgPoint: X coordinate is NaN " +
                "(TimedPoint: x=${point.x}, y=${point.y}, time=${point.timestamp})"
        }
        require(!point.x.isInfinite()) {
            "Cannot create SvgPoint: X coordinate is infinite (${point.x}) " +
                "(TimedPoint: x=${point.x}, y=${point.y}, time=${point.timestamp})"
        }
        require(!point.y.isNaN()) {
            "Cannot create SvgPoint: Y coordinate is NaN " +
                "(TimedPoint: x=${point.x}, y=${point.y}, time=${point.timestamp})"
        }
        require(!point.y.isInfinite()) {
            "Cannot create SvgPoint: Y coordinate is infinite (${point.y}) " +
                "(TimedPoint: x=${point.x}, y=${point.y}, time=${point.timestamp})"
        }
